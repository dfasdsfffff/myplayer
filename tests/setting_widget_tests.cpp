...
int main(int argc, char* argv[])
{
    ...
    if (!Expect(volume && apply && ok && cancel, "settings window exposes playback controls and actions"))
        return 1;

    volume->setValue(2.0);
    apply->click();
    if (!Expect(appliedCount == 1 && applied.volume == 1.0, "Apply emits sanitized preferences"))
        return 1;
