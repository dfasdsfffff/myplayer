...
- [ ] Write failing tests for ref-counted AVFrame lifetime, Y/U/V plane strides, odd-size rejection/fallback, and BGRA compatibility.
- [ ] For `AV_PIX_FMT_YUV420P` and formats safely convertible to it, retain/ref or convert into a pooled YUV420P frame. Keep the existing BGRA converter for unsupported formats and renderer fallback.
- [ ] In `Show`, create `SDL_PIXELFORMAT_IYUV` textures and call `SDL_UpdateYUVTexture`; use existing BGRA texture code for fallback. Recreate textures only when format or dimensions change.
- [ ] Add a runtime diagnostic showing active presentation format. Verify subtitles remain a separate blended overlay.
- [ ] Benchmark identical 1080p and 4K clips before/after, recording CPU and frame counts in the task completion note.
- [ ] Commit:
